...
export default function OrganisationDashboard({ user, profile }) {
  ...
  const t = {
    ...
    primary: "#2f6fed", // calmer enterprise blue
    primary2: "#1f5fe2",
    danger: "#e25555",
    danger2: "#cf3f3f",
    success: "#1f9d6a",
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // fetch members sequentially (simple + safe)
      const mems = {};
      for (let org of list) {
  ...
  const copyPin = async (pin) => {
    try {
      ...
      showToast({ tone: "success", icon: "📋", title: "Copied", message: "PIN copied to clipboard" });
    } catch {
      showToast({ tone: "danger", icon: "⚠️", title: "Copy failed", message: "Clipboard permission blocked" });
    }
  };

  const toggleExpand = (id) => setExpanded((prev) => ({ ...prev, [id]: !prev[id] }));
  const togglePin = (id) => setPinVisible((prev) => ({ ...prev, [id]: !prev[id] }));
  ...
  const runConfirm = async () => {
    ...
    try {
      if (intent === "delete") {
        ...
        await loadData();
        showToast({ tone: "success", icon: "🗑️", title: "Deleted", message: "Workspace removed" });
      } else {
        setBusy((b) => ({ ...b, leavingOrgId: org.id }));
        await leaveOrganisation(org.id, user.id);
        await loadData();
        showToast({ tone: "info", icon: "🚪", title: "Left workspace", message: "You left the workspace" });
      }
    } catch (err) {
      showToast({ tone: "danger", icon: "⚠️", title: "Action failed", message: safeErrorMessage(err) });
    } finally {
    ...
    <main style={styles.page}>
      ...
      >
        <div style={{ display: "grid", gap: 10 }}>
          <button
            style={{ ...styles.btn, ...styles.btnPrimary, width: "100%", display: "flex", gap: 10, justifyContent: "center", alignItems: "center" }}
            onClick={() => {
              setSearch("");
              setSortBy("name_asc");
              showToast({ tone: "success", icon: "🧼", title: "Reset", message: "Search + sort reset" });
              setShowQuick(false);
            }}
          >
            Reset search + sort
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (!first) {
                showToast({ tone: "info", icon: "ℹ️", title: "No workspaces", message: "Create or join one first" });
                return;
              }
              try {
      ...
      <Modal
        ...
        onClose={() => setConfirm({ open: false, title: "", subtitle: "", intent: "", org: null })}
        footer={
          <>
            <button
              style={styles.btn}
              onClick={() => setConfirm({ open: false, title: "", subtitle: "", intent: "", org: null })}
            >
              Cancel
            </button>
            <button
              style={{ ...styles.btn, ...(confirm.intent === "delete" ? styles.btnDanger : styles.btnPrimary) }}
              onClick={runConfirm}
              disabled={
                (confirm.intent === "delete" && busy.deletingOrgId === confirm.org?.id) ||
                (confirm.intent === "leave" && busy.leavingOrgId === confirm.org?.id)
              }
            >
      ...
      >
        <div style={{ color: "rgba(17,24,39,0.75)", fontSize: 13, fontWeight: 850, lineHeight: 1.4 }}>
          {confirm.intent === "delete" ? (
            <>
              This action is <strong>permanent</strong>. If you want to keep it, cancel and remove members instead.
            </>
          ) : (
            <>
        ...
        <div style={styles.panelGrid}>
          <section style={styles.panel} aria-label="Create workspace">
            ...
            <div style={styles.formGridCreate}>
              ...
              <button
                id="createOrgBtn"
                style={{ ...styles.btn, ...styles.btnPrimary, display: "flex", gap: 10, alignItems: "center", justifyContent: "center" }}
                disabled={busy.create}
                onClick={async () => {
                  if (!newOrgName.trim()) {
                    showToast({ tone: "danger", icon: "⚠️", title: "Missing name", message: "Enter a workspace name" });
                    return;
                  }
                  try {
                    ...
                    setNewOrgName("");
                    await loadData();
                    showToast({ tone: "success", icon: "✅", title: "Created", message: `PIN: ${data.pin}` });
                  } catch (err) {
                    showToast({ tone: "danger", icon: "⚠️", title: "Create failed", message: safeErrorMessage(err) });
                  } finally {
              ...
              <button
                ...
                onClick={async () => {
                  if (!joinName.trim() || !joinPin.trim()) {
                    showToast({ tone: "danger", icon: "⚠️", title: "Missing fields", message: "Enter workspace name and PIN" });
                    return;
                  }
                  try {
                    ...
                    setJoinPin("");
                    await loadData();
                    showToast({ tone: "success", icon: "🎉", title: "Joined", message: "Access granted" });
                  } catch (err) {
                    showToast({ tone: "danger", icon: "⚠️", title: "Join failed", message: safeErrorMessage(err) });
                  } finally {
              ...
              return (
                <HoverCard key={org.id} baseStyle={styles.card} hoverStyle={cardHover}>
                  <div style={styles.cardTop}>
                    <div style={{ display: "grid", gap: 8 }}>
                      ...
                      <div style={styles.badges}>
                        <Pill tone={isOwner ? "brand" : "neutral"}>{isOwner ? "Owner" : "Member"}</Pill>
                  ...
                  <div style={styles.actions}>
                    ...
                    {isOwner ? (
                      <button
                        style={{ ...styles.btn, background: "rgba(226,61,61,0.10)", borderColor: "rgba(226,61,61,0.18)", color: "rgba(207,63,63,1)" }}
                        disabled={isDeleting}
